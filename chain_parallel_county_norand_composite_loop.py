...
def multichain_run(i1, graph, chainlength, my_apportionment, poptol, my_electionproxy, composite, rsw, rmm, reg, rpp, datastruct, state, splitno, maxsplits, cutedgemax):
    hi_eg = 0.06 #spit out maps for anything with efficiency gap over this
    #cutedgemax=1.2 #factor above initial partition, cut edges allowed 
    random.seed(os.urandom(10)*i1) 
 #   poptol = 0.1  #min population deviation
...
#INITIAL PARTITION
    ...
    for part in chain:
        if part.good == 1:
            ...
            rpp.append(0) #(np.mean(pd.Series(polsby_popper(part.state))))  #depends on geometry of the partition only not on vote outcomes
            rsw.append(rsw_tmp)
            rmm.append(rmm_tmp)
            reg.append(reg_tmp)
...
if __name__ == '__main__':
    ...
    #exec(open("input_templates/TX_SEND_countyloop.py").read()) #read in input template
    #exec(open("input_templates/TX_USCD_countyloop.py").read()) #read in input template
    #exec(open("input_templates/PA_SEND_SEN12_countyloop.py").read()) #read in input template
    #exec(open("input_templates/NC_judge_EL12G_GV_countyloop.py").read()) #read in input template
    
   # exec(open("input_templates/WI_SEN_SEN16_countyloop.py").read()) #read in input template
    #exec(open("input_templates/MI_CD_PRES16_countyloop.py").read()) #read in input template
    #exec(open("input_templates/OH_CD_SEN16_countyloop.py").read()) #read in input template
    #exec(open("input_templates/OH_SEND_SEN16_countyloop.py").read()) #read in input template
    #exec(open("input_templates/FL_CD116_SEN16_countyloop.py").read()) #read in input template
    #exec(open("input_templates/MI_CD_PRES16_countyloop.py").read()) #read in input template
    exec(open("input_templates/GA_CD_SEN16_countyloop.py").read()) #read in input template
    ...
    #exec(open("input_templates/FL_CD116_SEN16_countyloop.py").read()) #read in input template
    #exec(open("input_templates/FL_SENDIST_SEN16_countyloop.py").read()) #read in input template
    #exec(open("input_templates/MA_CD_PRES16_countyloop.py").read()) #read in input template
#    exec(open("input_templates/MD_cSEND_PRES16_countyloop.py").read())
    exec(open("input_templates/FL_CD116_SEN16_countyloopa.py").read())
    #exec(open("input_templates/FL_SEND_SEN16_countyloop.py").read())
    #exec(open("input_templates/GA_CD_SEN16_countyloop.py").read())
    #exec(open("input_templates/GA_SEND_SEN16_countyloop.py").read())
#    exec(open("input_templates/MI_CD_PRES16_countyloop.py").read())
   # exec(open("input_templates/NC_send_EL12G_GV_countyloop.py").read())
   
    postfileadd ='a' #modifier to filenmame
    ...
    for loopind in range(len(maxsplitlist)):
        maxsplits = maxsplitlist[loopind]
        countysp = 'xsplitsrand ' +str(maxsplits)  #labels for graphs and output filenames
        cutedgemax = cutedgemaxlist[loopind]
    ...
    # RUN condense_datastruct.py after this to unpack the data structure and plot it
        print("finished splits = ", maxsplits, "\n")
        reg_clean_list.append(reg_clean)
        rmm_clean_list.append(rmm_clean)
        rsw_clean_list.append(rsw_clean)
        
        reg_clean_mean.append(np.mean(reg_clean))
        rmm_clean_mean.append(np.mean(rmm_clean))
        rsw_clean_mean.append(np.mean(rsw_clean))
        exec(open("condense_datastruct_noplot.py").read()) 
    
    #loop done so now archive the results
    the_data = {"reg_list": reg_clean_list, "rmm_list" : rmm_clean_list, 
                "rsw_list": rsw_clean_list, "reg_mean": reg_clean_mean,
                "rmm_mean" : rmm_clean_mean, "rsw_mean": rsw_clean_mean,
                "splitno": splitno_clean,
                "countysplits" : maxsplitlist, "cutedgemax" : cutedgemaxlist}
    outfn = './redist_data/listdat' + state + my_apportionment + '_'+ str(chainlength) + postfileadd
    pk.dump(the_data, open(outfn, "wb")) #write the_data to file outfn
