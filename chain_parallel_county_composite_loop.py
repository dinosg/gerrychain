...
#    poptol = 0.05  #min population deviation
...
#INITIAL PARTITION
    ...
    for part in chain:
        if part.good == 1:
            ...
            print(i1, total_splits(part.state), '\n')
            #splits don't depend on individual election results, only on partition so not in loop
...
if __name__ == '__main__':
    ...
    #exec(open("input_templates/PA_CD_2011_SEN12_countyloop.py").read()) #read in input template
...
#    exec(open("input_templates/FL_CD116_SEN16_countyloop.py").read()) #read in input template
    ...
    #exec(open("input_templates/MA_SEND_PRES16_countyloop.py").read()) #read in input template
    #exec(open("input_templates/MD_CD_PRES16_countyloop.py").read()) #read in input template
    #exec(open("input_templates/MD_SEND_PRES16_countyloop.py").read()) #read in input template
    dontfeedin = 0  #if set=0, feeds in data, otherwise skip
    poolsize=40
    chainlength=500
    corrlength=50
    postfileadd ='a'  #modifier to output file name
    ...
    for loopind in range(len(maxsplitlist)):
        ...
        ctx = get_context("forkserver")   #or spawn or forkserver or fork
        p = ctx.Pool(poolsize)
