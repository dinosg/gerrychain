...
def multichain_run(i1, graph, chainlength, my_apportionment, poptol, my_electionproxy, composite, rsw, rmm, reg, rpp, datastruct, state):
    cutedgemax = 1.2 #max cut edges allowed compared to initial partition
    hi_eg = 1.12  #spit out maps for anything with efficiency gap over this
    random.seed(os.urandom(10)*i1) 
    #poptol = 0.02  #min population deviation
...
if __name__ == '__main__':
    ...
    exec(open("input_templates/WI_SEN_SEN16.py").read()) #read in input template
    elections, composite = get_elections(state)
