...
def multichain_run(i1, graph, chainlength, my_apportionment,poptol,  my_electionproxy, my_electionproxy_alternate, rsw, rmm, reg, rpp, datastruct, state, splitno, maxsplits):
    hi_eg = 0.03  #spit out maps for anything with efficiency gap over this
...
#INITIAL PARTITION
    ...
    1.3*len(initial_partition["cut_edges"])
    )
...
if __name__ == '__main__':
    ...
    updated_vals = p.starmap(multichain_run, [(i1, graph_PA, chainlength, my_apportionment, poptol, my_electionproxy, my_electionproxy_alternate,
                                               rsw[i1], rmm[i1], reg[i1], rpp[i1], datastruct[i1], state, splitno[i1], maxsplits) for i1 in range(poolsize)])
