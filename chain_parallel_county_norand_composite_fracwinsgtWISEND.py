...
import time
from gerrychain import (GeographicPartition, Partition, Graph, MarkovChain_xtendedfracwinsgt,
...
def multichain_run(i1, graph, chainlength, my_apportionment, poptol, my_electionproxy, composite, rsw, rmm, reg, rpp, datastruct, state, splitno, maxsplits, cutedgemax, electionvol):
    hi_wins = 14#spit out maps for anything with measure over this
...
#INITIAL PARTITION
    ...
    if proposaltype == 'recom':
        chain = MarkovChain_xtendedfracwinsgt(
        proposal=proposal,
        ...
        election_composite = composite
        )
    elif proposaltype == 'flip'  : 
        chain = MarkovChain_xtendedfracwinsgt(
    ...
    for part in chain:
        if (part.good == 1) or (part.good == -1):
            ...
            rsw_tmp = calc_fracwins_comp(part.state,composite, electionvol)
    ...
    #        rpp.append(np.mean(pd.Series(polsby_popper(part.state))))  #depends on geometry of the partition only not on vote outcomes
            ...
            #cd.cd_gt(part.state,hi_wins,rsw_tmp ,state, my_apportionment,my_electionproxy, i1, '_cdgt')
            if (part.good == -1) or (part.good == 1):
                
                print('worker ', i1, 'rsw valued = ' ,rsw_tmp)
                cd.cd_gt(part.state,hi_wins-100, rsw_tmp,state, my_apportionment,my_electionproxy, i1, '_cdgt')
...
#    exec(open("./input_templates/PA2000_SEND_test_waxenberg.py").read()) 
#    exec(open("./input_templates/WI_SEN_SEN16_2020.py").read())  
    exec(open("./input_templates/WI_SEN_census_2020.py").read())  
    countysp = 'xsplits_' +str(maxsplits)  #labels for graphs and output filenames
    normalize=''
    elections, composite = get_elections(state)
    ...
    updated_vals = p.starmap(multichain_run, [(i1, graph_PA, chainlength, my_apportionment, poptol, my_electionproxy, composite,
                                               rsw[i1], rmm[i1], reg[i1], rpp[i1], datastruct[i1], state, splitno[i1], maxsplits, cutedgemax, electionvol) for i1 in range(poolsize)])
