...
import time
from gerrychain import (GeographicPartition, Partition, Graph, MarkovChain_xtended_ltpolish_fracs,
...
def multichain_run(i1, graph, chainlength, my_apportionment, poptol, my_electionproxy, composite, rsw, rmm, reg, rpp, datastruct, state, splitno, maxsplits, min_seats, cutedgemax, electionvol):
    hi_wins = 11#spit out maps for anything with efficiency gap over this
...
#INITIAL PARTITION
    ...
    chain = MarkovChain_xtended_ltpolish_fracs(
    proposal=proposal,
    ...
    #        rpp.append(np.mean(pd.Series(polsby_popper(part.state))))  #depends on geometry of the partition only not on vote outcomes
            ...
            cd.cd_lt(part.state,hi_wins,rsw_tmp ,state, my_apportionment,my_electionproxy, i1, '_cdgt')
...
#    exec(open("input_templates/PA_CD_2011_SEN12_countyloop.py").read()) 
   # exec(open("input_templates/PA_SEND_SEN12_countyloop.py").read()) 
    ...
    exec(open("input_templates/OH_CD_SEN16_20_shrink.py").read()) 
    ...
    updated_vals = p.starmap(multichain_run, [(i1, graph_PA, chainlength, my_apportionment, poptol, my_electionproxy, composite,
                                               rsw[i1], rmm[i1], reg[i1], rpp[i1], datastruct[i1], state, splitno[i1], maxsplits, min_seats, cutedgemax, electionvol) for i1 in range(poolsize)])
