...
def multichain_run(i1, graph, chainlength, my_apportionment, poptol, my_electionproxy, composite, rsw, rmm, reg, rpp, datastruct, state, splitno, maxsplits, cutedgemax, electionvol):
    hi_wins = 8#spit out maps for anything with efficiency gap over this
...
#INITIAL PARTITION
    ...
    #        rpp.append(np.mean(pd.Series(polsby_popper(part.state))))  #depends on geometry of the partition only not on vote outcomes
            ...
            cd.cd_gt(part.state,hi_wins,rsw_tmp ,state, my_apportionment,my_electionproxy, i1, '_cdgt'+proposaltype)
...
#    exec(open("input_templates/PA_CD_2011_SEN12_countyloop.py").read()) 
   # exec(open("input_templates/PA_SEND_SEN12_countyloop.py").read()) 
    ...
    exec(open("./input_templates/PA_districtr17shrink1_2.py").read()) 
    countysp = 'xsplitsrand ' +str(maxsplits)  #labels for graphs and output filenames
    normalize=''
    elections, composite = get_elections(state)
   
    #read in data file here:
    #CREATE graph_PA IN UNDERLYING SCRIPT splice_assignment_fn.py
    ...
    if dontfeedin == 0 or not( 'graph_PA' in globals()):
     ...
     #use geopandas to read data EVERY TIME
